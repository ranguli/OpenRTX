...
#error Unsupported vertical resolution!
    ...
    static const uint16_t line1_pos = top_h + top_pad + line1_h
                                    - small_line_v_pad - text_v_offset;
    static const uint16_t line2_pos = top_h + top_pad + line1_h + line2_h
                                    - small_line_v_pad - text_v_offset;
    ...
    static const uint16_t line4_pos = top_h + top_pad + line1_h + line2_h
                                    ...
                                    - text_v_offset;
    static const uint16_t line3_large_pos = top_h + top_pad + line1_h + line2_h
                                          + line3_large_h - big_line_v_pad
                                          - text_v_offset;
    static const uint16_t bottom_pos = CONFIG_SCREEN_HEIGHT - bottom_pad
        ...
        switch (state.ui_screen) {
                ...
                if (ui_state.edit_mode) {
                    ...
                    if (state.channel.mode == OPMODE_M17) {
                        if (msg.keys & KEY_HASH) {
                            ...
                            *sync_rtx = true;
                            ...
                            InputResult result = callsignInput.handleKey(uiCtx,
                                                                         event);
                    ...
                    if (msg.keys & KEY_ENTER) {
                        ...
                        if (state.channel.mode == OPMODE_M17) {
                            ...
                            callsignInput.start(ui_state.new_callsign, 9,
                                                t9CallsignSymbols,
                                                layout.input_font);
                            vp_announceM17Info(NULL, ui_state.edit_mode,
                                               queueFlags);
            ...
            case SETTINGS_M17:
                if (ui_state.edit_mode) {
                    switch (ui_state.menu_selected) {
                        case M17_CALLSIGN:
                            if (msg.long_press && (msg.keys & KEY_F1)
                                && (state.settings.vpLevel > vpBeep)) {
                                ...
                                InputResult result =
                                    callsignInput.handleKey(uiCtx, event);
                                if (result == InputResult::Confirmed) {
                                    strncpy(state.settings.callsign,
                                            ui_state.new_callsign, 10);
                                    ui_state.edit_mode = false;
                                    vp_announceBuffer(
                                        &currentLanguage->callsign, false, true,
                                        state.settings.callsign);
                                } else if (result == InputResult::Cancelled) {
                                    ui_state.edit_mode = false;
                                    vp_announceBuffer(
                        ...
                        case M17_METATEXT:
                            if (msg.long_press && (msg.keys & KEY_F1)
                                ...
                                InputResult result =
                                    messageInput.handleKey(uiCtx, event);
                                if (result == InputResult::Confirmed) {
                                    ...
                                    ui_state.edit_mode = false;
                                    vp_announceBuffer(
                                        &currentLanguage->metaText, false, true,
                    ...
                    if (msg.keys & KEY_ENTER) {
                        ...
                        if (ui_state.menu_selected == M17_METATEXT) {
                            ...
                            messageInput.start(ui_state.new_message, 52,
                                               t9TextSymbols,
                                               layout.message_font);
