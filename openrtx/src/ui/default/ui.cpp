...
#ifdef CONFIG_GPS
class SettingsGpsScreen          : public Screen { public: bool handleInput(event_t) override; void render() override; };
#endif
...
class SettingsReset2DefaultsScreen   : public Screen { public: bool handleInput(event_t) override; void render() override; };
class LowBatScreen                   : public Screen { public: bool handleInput(event_t) override; void render() override; };

// ─── Static screen instances ────────────────────────────────────────────────

static VfoScreen               vfoScreen;
static VfoInputScreen          vfoInputScreen;
static MemScreen               memScreen;
static MenuTopScreen           menuTopScreen;
static MenuBankScreen          menuBankScreen;
static MenuChannelScreen       menuChannelScreen;
static MenuContactsScreen      menuContactsScreen;
#ifdef CONFIG_GPS
static MenuGpsScreen           menuGpsScreen;
#endif
static MenuSettingsScreen      menuSettingsScreen;
static MenuBackupRestoreScreen menuBackupRestoreScreen;
static MenuBackupScreen        menuBackupScreen;
static MenuRestoreScreen       menuRestoreScreen;
static MenuInfoScreen          menuInfoScreen;
static MenuAboutScreen         menuAboutScreen;
#ifdef CONFIG_RTC
static SettingsTimedateScreen    settingsTimedateScreen;
static SettingsTimedateSetScreen settingsTimedateSetScreen;
#endif
static SettingsDisplayScreen     settingsDisplayScreen;
#ifdef CONFIG_GPS
static SettingsGpsScreen         settingsGpsScreen;
#endif
static SettingsRadioScreen       settingsRadioScreen;
#ifdef CONFIG_M17
static SettingsM17Screen         settingsM17Screen;
#endif
static SettingsFmScreen              settingsFmScreen;
static SettingsAccessibilityScreen   settingsAccessibilityScreen;
static SettingsReset2DefaultsScreen  settingsReset2DefaultsScreen;
static LowBatScreen                  lowBatScreen;
...
void VfoScreen::render()                    { _ui_drawMainVFO(&ui_state); }
void VfoInputScreen::render()               { _ui_drawMainVFOInput(&ui_state); }
void MemScreen::render()                    { _ui_drawMainMEM(&ui_state); }
void MenuTopScreen::render()                { _ui_drawMenuTop(&ui_state); }
void MenuBankScreen::render()               { _ui_drawMenuBank(&ui_state); }
void MenuChannelScreen::render()            { _ui_drawMenuChannel(&ui_state); }
void MenuContactsScreen::render()           { _ui_drawMenuContacts(&ui_state); }
#ifdef CONFIG_GPS
void MenuGpsScreen::render()                { _ui_drawMenuGPS(); }
#endif
void MenuSettingsScreen::render()           { _ui_drawMenuSettings(&ui_state); }
void MenuBackupRestoreScreen::render()      { _ui_drawMenuBackupRestore(&ui_state); }
void MenuBackupScreen::render()             { _ui_drawMenuBackup(&ui_state); }
void MenuRestoreScreen::render()            { _ui_drawMenuRestore(&ui_state); }
void MenuInfoScreen::render()               { _ui_drawMenuInfo(&ui_state); }
void MenuAboutScreen::render()              { _ui_drawMenuAbout(&ui_state); }
#ifdef CONFIG_RTC
void SettingsTimedateScreen::render()       { _ui_drawSettingsTimeDate(); }
void SettingsTimedateSetScreen::render()    { _ui_drawSettingsTimeDateSet(&ui_state); }
#endif
void SettingsDisplayScreen::render()        { _ui_drawSettingsDisplay(&ui_state); }
#ifdef CONFIG_GPS
void SettingsGpsScreen::render()            { _ui_drawSettingsGPS(&ui_state); }
#endif
void SettingsRadioScreen::render()          { _ui_drawSettingsRadio(&ui_state); }
#ifdef CONFIG_M17
void SettingsM17Screen::render()            { _ui_drawSettingsM17(&ui_state); }
#endif
void SettingsFmScreen::render()             { _ui_drawSettingsFM(&ui_state); }
...
bool VfoScreen::handleInput(event_t ev)
{
    ...
    if(state.txDisable) { state.txDisable = false; *gSyncRtx = true; }
    bool skipLock = (state.channel.mode == OPMODE_FM) && (msg.keys == KEY_HASH);
    if(ui_state.input_locked && !skipLock) return true;
...
#endif
            {
                if(!state.tone_enabled) { state.tone_enabled = true; *gSyncRtx = true; }
            }
        }
...
bool VfoInputScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    ...
    else if(msg.keys & KEY_UP || msg.keys & KEY_DOWN)
    {
        ...
        _ui_fsm_insertVFONumber(msg, gSyncRtx);
    return true;
}

bool MemScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    ...
    if(state.txDisable) { state.txDisable = false; *gSyncRtx = true; }
    if(ui_state.input_locked) return true;
    ...
    else
    {
        ...
        {
            _ui_fsm_loadChannel(state.channel_index + 1, gSyncRtx);
...
bool MenuBankScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
...
bool MenuChannelScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
...
bool MenuContactsScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
...
#ifdef CONFIG_GPS
bool MenuGpsScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
...
bool MenuBackupRestoreScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    ...
    if(msg.keys & KEY_UP || msg.keys & KNOB_LEFT)
        _ui_menuUp(backup_restore_num);
    else if(msg.keys & KEY_DOWN || msg.keys & KNOB_RIGHT)
        _ui_menuDown(backup_restore_num);
    else if(msg.keys & KEY_ENTER)
    {
        switch(ui_state.menu_selected)
        {
...
bool MenuBackupScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    if(msg.keys & KEY_ESC) _ui_menuBack(&menuTopScreen);
    return true;
}

bool MenuRestoreScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
...
bool MenuAboutScreen::handleInput(event_t ev)
{
    ...
    if(msg.keys & KEY_UP || msg.keys & KNOB_LEFT)
    {
        if(ui_state.menu_selected > 0) ui_state.menu_selected -= 1;
    }
    else if(msg.keys & KEY_DOWN || msg.keys & KNOB_RIGHT)
        ui_state.menu_selected += 1;
    else if(msg.keys & KEY_ESC)
...
#ifdef CONFIG_RTC
bool SettingsTimedateScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
...
bool SettingsTimedateSetScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    ...
    if(msg.keys & KEY_ENTER)
    {
        if(ui_state.input_position < TIMEDATE_DIGITS) return true;
...
#ifdef CONFIG_GPS
bool SettingsGpsScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
        ...
        msg.keys & KEY_UP || msg.keys & KNOB_RIGHT)))
    {
        ...
        {
            case G_ENABLED:
                if(state.settings.gps_enabled) state.settings.gps_enabled = 0;
                else state.settings.gps_enabled = 1;
                vp_announceSettingsOnOffToggle(&currentLanguage->gpsEnabled, queueFlags,
                                               state.settings.gps_enabled);
                break;
#ifdef CONFIG_RTC
            case G_SET_TIME:
                state.settings.gpsSetTime = !state.settings.gpsSetTime;
                vp_announceSettingsOnOffToggle(&currentLanguage->gpsSetTime, queueFlags,
...
bool SettingsRadioScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    ...
    {
        ui_state.edit_mode = true;
        if(ui_state.menu_selected == R_OFFSET) ui_state.new_offset = 0;
        ui_state.input_position = 0;
    }
    else if(msg.keys & KEY_ESC)
        _ui_menuBack(&menuSettingsScreen);
        ...
        switch(ui_state.menu_selected)
        {
            ...
            case M17_METATEXT:
                if(msg.keys & KEY_ENTER)
                {
                    ...
                    ui_state.edit_mode = false;
                    vp_announceBuffer(&currentLanguage->metaText, false, true, state.settings.M17_meta_text);
                }
                else if(msg.keys & KEY_ESC)
                {
                    ui_state.edit_message = false;
                ...
                {
                    vp_announceBuffer(&currentLanguage->metaText, true, true, ui_state.new_message);
                    gF1Handled = true;
                }
                break;
            case M17_CAN:
...
bool SettingsFmScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
    ...
    if(ui_state.edit_mode)
    {
        if(msg.keys & KEY_ESC) ui_state.edit_mode = false;
...
bool SettingsAccessibilityScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
       ...
       (msg.keys & KEY_DOWN || msg.keys & KNOB_LEFT)))
    {
        switch(ui_state.menu_selected)
        {
            case A_MACRO_LATCH: _ui_changeMacroLatch(false); break;
            case A_LEVEL:       _ui_changeVoiceLevel(-1);    break;
            case A_PHONETIC:    _ui_changePhoneticSpell(false); break;
            default: break;
        }
    }
    else if(msg.keys & KEY_RIGHT || (ui_state.edit_mode &&
            (msg.keys & KEY_UP || msg.keys & KNOB_RIGHT)))
    {
        switch(ui_state.menu_selected)
        {
            case A_MACRO_LATCH: _ui_changeMacroLatch(true);  break;
            case A_LEVEL:       _ui_changeVoiceLevel(1);     break;
            case A_PHONETIC:    _ui_changePhoneticSpell(true); break;
            default: break;
        }
    }
    else if(msg.keys & KEY_UP || msg.keys & KNOB_LEFT)
...
bool LowBatScreen::handleInput(event_t ev)
{
    if(ev.type != EVENT_KBD) return true;
    kbd_msg_t msg;
    msg.value = ev.payload;
