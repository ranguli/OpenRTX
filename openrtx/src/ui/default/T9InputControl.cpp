...
const char *const t9CallsignSymbols[] = { "0 ",   "1",     "ABC2", "DEF3",
                                          "GHI4", "JKL5",  "MNO6", "PQRS7",
                                          "TUV8", "WXYZ9", "-/",   "" };

const char *const t9TextSymbols[] = { " 0",        ",.?1",      "abc2ABC",
...
InputResult T9InputControl::handleKey(UIContext &ctx, event_t event)
{
    ...
    if (msg.keys & KEY_UP || msg.keys & KEY_DOWN || msg.keys & KEY_LEFT
        || msg.keys & KEY_RIGHT) {
        handleDelete();
...
void T9InputControl::draw(UIContext &ctx, bool overlay)
{
    ...
    if (overlay) {
        ...
        uint16_t rect_height =
            (CONFIG_SCREEN_HEIGHT - (ctx.layout.top_h + ctx.layout.bottom_h))
            / 2;
        point_t rect_origin = {
    ...
    gfx_printLine(1, 1, ctx.layout.top_h,
                  ...
                  ctx.layout.horizontal_pad, font_, TEXT_ALIGN_CENTER,
                  color_white, buf_);
}
